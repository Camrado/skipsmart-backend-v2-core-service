...
internal sealed class DateTimeProvider : IDateTimeProvider {
    ...
    public DateOnly SemesterStartDate {
        get {
            if (TodayInBaku < new DateOnly(2026, 1, 10)) {
                return new DateOnly(2025, 9, 2);
            }
            else {
                return new DateOnly(2026,1,19);
            }
        }
    }

    public DateOnly FirstSemesterStartDate => new DateOnly(2025, 9, 2);

    public DateOnly SecondSemesterStartDate => new DateOnly(2026, 1, 19);
}